...
type Signer struct {
	...
	// AuthenticationMethod specifies header vs query string signing
	// Defaults to AuthenticationMethodHeader (zero value)
...
func (s *Signer) Do() error {
	...
	if s.AuthenticationMethod == v4.AuthenticationMethodQueryString {
...
func (s *Signer) setRequiredHeaders() {
	...
	// AuthenticationMethodHeader - these values are passed in the query string when using AuthenticationMethodQueryString
	if s.AuthenticationMethod == v4.AuthenticationMethodHeader {
