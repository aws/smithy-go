 ...
 */
@SmithyInternalApi
public final class ServerSymbolVisitor implements SymbolProvider, ShapeVisitor<Symbol> {
    ...
    public ServerSymbolVisitor(Model model, GoSettings settings) {
        this.model = model;
    ...
    public Symbol documentShape(DocumentShape shape) {
        throw new CodegenException("Document shapes are not supported in Go Server Codegen: `" + shape + "`");
    }
    ...
    public Symbol operationShape(OperationShape shape) {
        ...
        return SymbolUtils.createPointableSymbolBuilder(shape, name, typesPackageName)
                .definitionFile(String.format("./types/operations.go", name))
                .build();
    }
    ...
    public Symbol serviceShape(ServiceShape shape) {
        return symbolBuilderFor(shape, "Server", rootModuleName)
                .definitionFile("./feat_svcgen.go")
                .build();
    }
    ...
    public Symbol structureShape(StructureShape shape) {
        ...
        if (shape.getId().getNamespace().equals(CodegenUtils.getSyntheticTypeNamespace())) {
            ...
            if (boundOperationName.isPresent()) {
                return symbolBuilderFor(shape, name, typesPackageName)
                        .definitionFile("./types/operations.go")
                        .build();
            }
        }
