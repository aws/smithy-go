...
public class FnGenerator {
    ...
    GoWriter.Writable generate(FunctionDefinition fnDef, List<Expr> fnArgs) {
        var goFn = fnProvider.fnFor(fnDef.id());

        List<GoWriter.Writable> writableFnArgs = new ArrayList<>();
        fnArgs.forEach((expr) -> {
            writableFnArgs.add(new ExprGenerator(scope).generate(expr));
        });
