...
final class ExprGenerator {
    private final Scope scope;

    ExprGenerator(Scope scope) {
        this.scope = scope;
    }

    public GoWriter.Writable generate(Expr expr) {
        var exprOrRef = expr;
        var optExprIdent = scope.getIdent(expr);
        if (optExprIdent.isPresent()) {
            exprOrRef = new Ref(Identifier.of(optExprIdent.get()), SourceLocation.NONE);
        }
        return exprOrRef.accept(new ExprGeneratorVisitor(scope));
    }

    private record ExprGeneratorVisitor(Scope scope) implements ExprVisitor<GoWriter.Writable> {
        ...
        @Override
        public GoWriter.Writable visitRef(Ref ref) {
    ...
    private record LiteralGeneratorVisitor(Scope scope) implements Literal.Vistor<GoWriter.Writable> {
        ...
        @Override
        public GoWriter.Writable visitStr(Template value) {
            Stream<GoWriter.Writable> parts = value.accept(
                    new TemplateGeneratorVisitor((expr) -> new ExprGenerator(scope).generate(expr)));
        ...
        @Override
        public GoWriter.Writable visitObject(Map<Identifier, Literal> members) {
            return goBlockTemplate("map[string]interface{}{", "}",
                    (w) -> {
                        members.forEach((k, v) -> {
                            w.write("$S: $W,", k.getName().toString(), new ExprGenerator(scope).generate(v));
                        });
                    });
        }
        ...
        public GoWriter.Writable visitTuple(List<Literal> members) {
            return goBlockTemplate("[]interface{}{", "}",
                    (w) -> {
                        members.forEach((v) -> w.write("$W,", new ExprGenerator(scope).generate(v)));
                    });
        }

        @Override
        public GoWriter.Writable visitInt(int i) {
            return goTemplate(String.valueOf(i));
        }
    }

    private record TemplateGeneratorVisitor(
            Function<Expr, GoWriter.Writable> generator) implements TemplateVisitor<GoWriter.Writable> {
        ...
        @Override
        public GoWriter.Writable visitSingleDynamicTemplate(Expr expr) {
        ...
        @Override
        public GoWriter.Writable visitDynamicElement(Expr expr) {
            return (GoWriter w) -> {
