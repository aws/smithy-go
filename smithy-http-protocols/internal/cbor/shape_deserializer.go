...
func (d *ShapeDeserializer) remaining() []byte {
	return d.p[d.off:]
}
...
func (d *ShapeDeserializer) ReadNil(s *smithy.Schema) (bool, error) {
	...
	if d.peekMajor() == majorType7 {
		minor := d.peekMinor()
		if minor == major7Nil || minor == 23 { // nil or undefined
			d.off++
			return true, nil
		}
	}
...
func (d *ShapeDeserializer) ReadBool(s *smithy.Schema, v *bool) error {
	if d.off >= len(d.p) {
		return fmt.Errorf("unexpected end of CBOR data")
	}
	if d.peekMajor() != majorType7 {
...
func (d *ShapeDeserializer) ReadTime(schema *smithy.Schema, v *time.Time) error {
	// rpcv2Cbor: always epoch-seconds in tag 1
	if d.off >= len(d.p) {
		return fmt.Errorf("unexpected end of CBOR data")
	}
	...
	// The tagged value can be int or float
	if d.off >= len(d.p) {
		return fmt.Errorf("unexpected end of CBOR data")
	}
	major := d.peekMajor()
...
func (d *ShapeDeserializer) ReadList(s *smithy.Schema) error {
	...
	if d.peekMinor() == minorIndefinite {
		d.off++
		d.head.Push(&deserListCtx{remaining: -1}) // indefinite
		return nil
	}
	count, err := d.readArg()
	if err != nil {
...
func (d *ShapeDeserializer) ReadMap(s *smithy.Schema) error {
	...
	if d.peekMinor() == minorIndefinite {
		d.off++
		d.head.Push(&deserMapCtx{remaining: -1}) // indefinite
		return nil
	}
	count, err := d.readArg()
	if err != nil {
...
func (d *ShapeDeserializer) ReadStruct(s *smithy.Schema) error {
	...
	if d.peekMinor() == minorIndefinite {
		d.off++
		d.head.Push(&deserStructCtx{schema: s, remaining: -1})
		return nil
	}
	count, err := d.readArg()
	if err != nil {
...
func (d *ShapeDeserializer) ReadUnion(s *smithy.Schema) (*smithy.Schema, error) {
	if _, ok := d.head.Top().(*deserUnionCtx); !ok { // first call: open the map
		if d.off >= len(d.p) {
		...
		if d.peekMinor() == minorIndefinite {
			d.off++
			d.head.Push(&deserUnionCtx{schema: s, remaining: -1})
		} else {
			count, err := d.readArg()
			if err != nil {
				return nil, err
			}
			d.head.Push(&deserUnionCtx{schema: s, remaining: int(count)})
		}
	}

	uc := d.head.Top().(*deserUnionCtx)
	for {
		if uc.remaining == -1 {
			if d.off < len(d.p) && d.p[d.off] == 0xff {
				d.off++
...
// ReadNil is already defined above.

// skip skips over the next CBOR value.
func (d *ShapeDeserializer) skip() error {
	...
	case majorType7:
		minor := d.peekMinor()
		// 25=float16(2B), 26=float32(4B), 27=float64(8B), else=0B
		n := 0
...
type deserListCtx struct {
	remaining int
}

type deserMapCtx struct {
	remaining int
}

type deserStructCtx struct {
	schema    *smithy.Schema
	remaining int
}

type deserUnionCtx struct {
	schema    *smithy.Schema
	remaining int
}
